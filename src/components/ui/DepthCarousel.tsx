...
export type DepthCarouselItem =
  | string
  | { image: string; alt?: string; label?: string; sublabel?: string; badge?: string };
...
const normalizeItem = (it: DepthCarouselItem) =>
  typeof it === 'string' ? { image: it, alt: '', label: '', sublabel: '', badge: '' } : it;
        ...
        {data.map((item, i) => (
          ...
          >
            ...
            />
              ...
              <span className="pointer-events-none absolute inset-0 flex flex-col justify-end p-5 bg-gradient-to-t from-black/85 via-black/20 to-transparent">
                {item.badge && (
                  <span className="text-xs font-semibold tracking-wide text-white/70 mb-1">{item.badge}</span>
                )}
                {item.label && (
                  <span className="text-2xl font-bold text-white leading-tight">{item.label}</span>
                )}
                {item.sublabel && (
                  <span className="text-xs text-white/60 mt-1">{item.sublabel}</span>
                )}
              </span>
            )}
          </div>
