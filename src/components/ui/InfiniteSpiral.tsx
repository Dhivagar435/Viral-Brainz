// InfiniteSpiral.tsx (UPDATED)
import { useEffect, useMemo, useRef, type CSSProperties, type PointerEvent as ReactPointerEvent, type ReactNode } from 'react';
...
export interface InfiniteSpiralProps {
  ...
  grayscale?: number;
  className?: string;
  renderCard?: ((item: InfiniteSpiralItem, index: number) => ReactNode) | undefined;
}
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" role="list" aria-label="Infinite spiral gallery">
        {normalizedItems.map((item, index) => {
          const content = renderCard ? renderCard(item, index) : (
            <img
